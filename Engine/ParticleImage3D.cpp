...
ParticleImage3D::ParticleImage3D(Vector3 pos, TextureHandle texHandle, RRect texRect, Vector2 texAnchor, Color color, float angle, float rotSpeed, Vector3 inVelocity, Vector3 stayVelocity, Vector3 outVelocity, Vector2 inSize, Vector2 staySize, Vector2 outSize, float inTime, float stayTime, float outTime)
...
void ParticleImage3D::Spawn(Vector3 pos, TextureHandle texHandle, RRect texRect, Vector2 texAnchor, Color color, float angle, float rotSpeed, Vector3 inVelocity, Vector3 stayVelocity, Vector3 outVelocity, Vector2 inSize, Vector2 staySize, Vector2 outSize, float inTime, float stayTime, float outTime) {
	ParticleManager::Register(std::make_shared<ParticleImage3D>(pos, texHandle, texRect, texAnchor, color, angle, rotSpeed, inVelocity, stayVelocity, outVelocity, inSize, staySize, outSize, inTime, stayTime, outTime));
}
